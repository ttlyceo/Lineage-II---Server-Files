...
import gabriel.phantomEngine.util.PhantomPlayerUtils;
...
public class ChatTell implements IChatHandler
{
	...
	{
		if (!ZeuS.isActivePIN(activeChar) && !activeChar.isPhantom())
		{
			return;
		}
		
		if (!ZeuS.canChatWisp(activeChar) && !activeChar.isPhantom())
		{
			return;
		}
		...
		if ((receiver != null) && !receiver.isSilenceMode(activeChar.getObjectId()))
		{
            ...
            if (receiver.isPhantom())
            {
                PhantomPlayerUtils.answerPlayers(activeChar, receiver, text);
            }
			...
			if (!BlockList.isBlocked(receiver, activeChar))
			{
				...
				ZeuS.isSummonWisp(receiver, activeChar);
			}
			else
			{
				activeChar.sendPacket(SystemMessageId.THE_PERSON_IS_IN_MESSAGE_REFUSAL_MODE);
