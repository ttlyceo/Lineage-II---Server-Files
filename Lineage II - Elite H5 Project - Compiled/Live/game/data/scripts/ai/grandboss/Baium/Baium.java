...
import gabriel.config.GrandBossesConfig;
...
public final class Baium extends AbstractNpcAI
{
	...
	int minutesOfWait = 10;
	private Future<?> announcer;
	private long spawn;

	private String getDateFromLong(long date){
		long milliToEnd = date - Calendar.getInstance().getTimeInMillis();
		...
		if (numSecs < 0 || numMins < 0 || numHours < 0) {
			return "now!";
		}
		return parseTime(numHours)+":"+parseTime(numMins)+":"+parseTime((int) numSecs);
	}
	...
	public String onAdvEvent(String event, L2Npc npc, L2PcInstance player)
	{
		switch (event)
		{
			...
			case "wakeUp":
			{
				...
				cal.add(Calendar.MINUTE, minutesOfWait);
				spawn = cal.getTimeInMillis();

				announcer = ThreadPoolManager.getInstance().scheduleAiAtFixedRate(() -> {
					for (L2PcInstance l2PcInstance : L2World.getInstance().getPlayers()) {

						l2PcInstance.sendPacket(new ExShowScreenMessage(1, -1, ExShowScreenMessage.MIDDLE_RIGHT, 0, 1, 0, 0, false, 1000, false, "Baium's Awakening: " + getDateFromLong(spawn)));

					}
				},1000,1000);
				startQuestTimer("REAL_WAKE_UP", minutesOfWait * 60000, npc, null);
				break;
			}
			case "REAL_WAKE_UP": {

				if(announcer != null){
					announcer.cancel(true);
				}
				announcer = null;
				if (getStatus() == ALIVE)
				{
