...
import gabriel.community.communityBosses.CommunityBosses;
...
{
	private static final int RAIDBOSS_MAINTENANCE_INTERVAL = 15000; // 30 sec
	...
	public boolean doDie(L2Character killer)
	{
		...
		if (player != null)
		{
			...
			CommunityBosses.getInstance().updateBossDeath(killer.getName(), this.getId());
			RaidManager.getInstance().onRaidDeath(this, player);
			RaidBossEventManager.getInstance().onDeath(this, player);
	...
	protected void startMaintenanceTask()
	{
		...
		_maintenanceTask = ThreadPoolManager.getInstance().scheduleGeneralAtFixedRate(() -> checkAndReturnToSpawn(), 30000, getMaintenanceInterval() + Rnd.get(5000));
	}
