...
import gabriel.events.raidbossEvent.manager.RaidBossEventManager;
import l2r.Config;
...
public abstract class L2Character extends L2Object implements ISkillsHolder
{
	...
	public final boolean isInsideZone(ZoneIdType zone)
	{
		...
		{
			case PVP:
				if (((instance != null) && instance.isPvPInstance()) || this.isInTownWarEvent())
				{
					return true;
				}
	...
	public void teleToLocation(int x, int y, int z, int heading, int instanceId, int randomOffset)
	{
		...
		if(isPlayer()) {
			if(!pvpZone.isInZone( getActingPlayer() )) {
				getActingPlayer().updatePvPFlag(0);
			}			
		}
	...
	public void doAttack(L2Character target)
	{
		...
		if (getActingPlayer() != null)
		{
			...
			else if (target.isInsidePeaceZone(getActingPlayer()) && !target.getActingPlayer().isInTownWarEvent())
			{
				getAI().setIntention(CtrlIntention.AI_INTENTION_ACTIVE);
				sendPacket(ActionFailed.STATIC_PACKET);
				return;
		...
		else if (isInsidePeaceZone(this, target) && !target.getActingPlayer().isInTownWarEvent())
		{
			getAI().setIntention(CtrlIntention.AI_INTENTION_ACTIVE);
			sendPacket(ActionFailed.STATIC_PACKET);
			return;
		...
		else
		{
			if (isPlayer() && (target instanceof L2RaidBossInstance) && ((L2RaidBossInstance) target).isEventRaid())
			{
				...
				RaidBossEventManager.getInstance().checkRaidAttack(getActingPlayer(), (L2RaidBossInstance) target);
			}
	...
	private void beginCast(L2Skill skill, boolean simultaneously, L2Character target, L2Object[] targets)
	{
		...
		if (target.isRaid() || target.isRaidMinion()) {
			if(Config.AUTO_FLAG_MOBSID.stream().anyMatch(e-> e==target.getId())){
				if(this.isPlayer()) {
		...
		if (isPlayable())
		{
			...
			if ((skill.getReferenceItemId() > 0) && (ItemData.getInstance().getTemplate(skill.getReferenceItemId()).getBodyPart() == L2Item.SLOT_DECO))
			{
				...
				{
					boolean isManaBlock = ZeuSManaBlock(item.getId());
					if (item.isEquipped())
					{
						if ((item.getMana() < item.useSkillDisTime()) && !isManaBlock)
						{
...
//			_castInterruptTime = -2 + GameTimeController.getInstance().getGameTicks() + (hitTime / GameTimeController.MILLIS_IN_TICK);
	...
	private boolean ZeuSManaBlock(int idItem)
	{
		if (Config.ZEUS_ITEM_WITHOUT_LIFETIME != null)
		{
			if (Config.ZEUS_ITEM_WITHOUT_LIFETIME.size() > 0)
			{
				return Config.ZEUS_ITEM_WITHOUT_LIFETIME.contains(idItem);
			}
		}
		return false;
	}
...
//		if ((skill == null) || isSkillDisabled(skill) || ((skill.getFlyType() == FlyType.CHARGE) && isMovementDisabled()))
	...
	public void onForcedAttack(L2PcInstance player)
	{
		...
		if ((player.getTarget() != null) && !player.getTarget().canBeAttacked() && !player.allowPeaceAttack())
		{
	...
	public boolean isInsidePeaceZone(L2Object attacker, L2Object target)
	{
		...
		if ((attacker instanceof L2Summon) && (target.getActingPlayer() != null) && target.getActingPlayer().isInTownWarEvent() && !target.getActingPlayer().getClient().isDetached())
		{
			return false;
		}
		...
		if ((targets.length > 0) && (escapeRange > 0))
		{
			...
			if (targetList.isEmpty() && !skill.isAura())
			{
				if (isPlayer())
				{
					...
					else if ((skipPeaceZone > 0) && ((L2PcInstance) this).allowPeaceAttack())
					{
