...
public final class RequestBuyItem extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		boolean isAIOMultisell = player.isAioMultisell();

		if (!player.isGM() && !isAIOMultisell)
		{
		...
		if ((merchant == null) && (!player.isGM() && !isAIOMultisell))
		{
			sendPacket(ActionFailed.STATIC_PACKET);
			return;
		}
		...
		//TODO GABRIEL REMOVE THIS FOLLOWING LINE IF KLAUS TAKES ANOTHER INTERFACE

		final L2BuyList temp = BuyListData.getInstance().getBuyList(_listId);
		player.sendPacket(new ExBuyList(temp, player, castleTaxRate + baseTaxRate));
		player.sendPacket(new ExBuySellList(player, castleTaxRate + baseTaxRate, true));
		player.setIsUsingAioMultisell(false);
